...
const iltifatlar = [
    ...
    'Josta✘e cıkma teklifi eder misin ?',
    'İğrenç İnsansın!',
...
module.exports = async (message) => {
    if (!message.guild || message.author.bot) return
      const owner = client.users.cache.get("941388757913985074");
    ...
    if (message.channel.id === config.channels.chat) {
      ...
      const ownerr = client.users.cache.get("941388757913985074");
    const afkembed = new MessageEmbed()
        .setColor(message.member.displayHexColor)
        .setAuthor(message.member.displayName)
        .setFooter("Developed by Josta✘")
        .setTimestamp()
