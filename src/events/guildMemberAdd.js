...
module.exports = async member => {
    ...
    if (kurulus > 604800000) {
        ...
        member.guild.channels.cache.get(config.channels.welcomechannel).send(`✧ Grae'ye Hoşgeldin $(member)!
        
        Hesabın ${zaman1} ${zaman2} önce oluşturulmuş
        
        Seninle beraber ailemiz **${member.guild.memberCount}** kişi sayısına ulaştı. Tagımızı alarak ailemize destek olabilirsin
        
        Sunucu kurallarımız <#934973785218617356> kanalında belirtilmiştir. Unutma sunucu içerisinde ki Ceza kurallarını okuduğunu varsayarak gerçekleştirilecek.
        
        Solda bulunan **✧ Gates of Grace** adlı teyit odalarimiza girip <@&${config.registration.staff}> rolüne sahip yetkililerimiz tarafından kayıt olabilirsiniz
       `);
    } else {
        member.setNickname(config.registration.susoeciosnickname);
        member.roles.add(config.registration.suspecios);
        member.setNickname(config.registration.autonickname);
        member.guild.channels.cache.get(config.channels.welcomechannel).send(`${member} Adlı kullanıcı sunucuya katıldı fakat hesabı **${zaman2}** önce açıldığı için şüpheli katagorisine düştü!`)
    }
}
